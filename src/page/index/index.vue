<template>
   <div :class="{'avue--collapse':isCollapse}" class="avue-contail" id="inspectContent">
      ...
      <div class="avue-layout">
         ...
         <div class="avue-main">
            ...
            <tags />
            <!-- 主体视图层 -->
            <el-scrollbar style="height:100%" ref="myScrollbar" class="el-menuscrollbar">
               <keep-alive>
...
import quickMenu from "@/page/index/quickMenu/index";
   ...
   methods: {
         ...
         this.$nextTick(() => {
            if (document.getElementById("inspectContent") != null) {
               if (self.$refs["myScrollbar"] != undefined) {
                  self.$refs[
                     "myScrollbar"
                  ].wrap.scrollTop = document.getElementById(
                     "inspectContent"
                  ).offsetTop;
               }
            }
      ...
      initWebSocket() {
         ...
         this.timer = setInterval(() => {
            try {
               self.stompClient.send("/app", "test");
            } catch (err) {
               console.log("断线了: " + err);
               self.connection();
            }
      ...
      getAllPatients() {
         ...
         //    var data = [];
         //    if (
         //       JSON.parse(sessionStorage.getItem("pigx-all_patients"))
         //          .dataType === "undefined"
         //    ) {
         //       data[0] = {};
         //    } else {
         ...
         //          .content;
         //    }
         //    this.$store.commit("SET_MESSAGE", data[0]);
         // });
      }
   }
};

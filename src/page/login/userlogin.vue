   ...
   methods: {
      ...
      handleLogin() {
         this.$refs.loginForm.validate(valid => {
            if (valid) {
               this.handleLoginLoading = true;
               this.$store
                  .dispatch("LoginByUsername", this.loginForm)
                  .then(res => {
                     this.$router.push({ path: this.tagWel.value }).then(() => {
                        this.refreshCode();
                        this.handleLoginLoading = false;
                     });
                  })
                  .catch(() => {
                     this.handleLoginLoading = false;
      ...
      handleLoginPost() {
         newLogin().then(res => {
            if (res.data.data.ismac) {
               this.userName = res.data.data.data;
               this.$router.push({ path: "/nologin", query: this.userName });
            }
         });
      }
   }

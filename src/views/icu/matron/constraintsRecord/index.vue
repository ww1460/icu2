   ...
   <div class="restrain">
      <basic-container  v-if="flag">
         <div>
                  ...
                  <el-form :inline="true" class="add">
                     <el-col :md="4" :xs="24">
                        <el-form-item label="患者姓名">
                          <el-select v-model="patientId" placeholder="患者姓名" size="small" @change="chosePatient($event,true)" clearable>
                              <el-option
            ...
            <el-dialog @close="closeDilog('form')" :fullscreen="isFullscreen" title="新增" :visible.sync="dialogFormVisible">
                  ...
                  <el-row>
                     ...
                     >
                        <el-form-item label="患者姓名" prop='patientId'>
                           <el-select style="width:100%" v-model="form.patientId">
                              <el-option
                                 v-for="item in patientList"
                                 :key="item.id"
                                 :value="item.patientId"
               ...
               <div class="ql-editor">
                  ...
                  <p style="display:flex">
                     ...
                     <span style="width:50%;height:50px;display:block">
                        ...
                        <input type="text" v-model="matter" id="matter" style="width: 50%;height:30px;border-top: 0;border-left:0;border-right:0;text-indent:5px;position:relative;top:6px;"/>
                     </span>
                  </p>
                  <p
                     style="font-weight: 100;font-size: 18px;text-align:right"
      ...
      <div v-if="autoFlag" class="autograph">
         <autograph />
      </div>
   </div>
</template>
...
import autograph from "../../patient/components/autograph";
import { dateFormat, calcDate } from "@/util/date";

export default {
   name: "constraintsRecord",
   components: {
      autograph,
   },
   watch: {
   ...
   data() {
      return {
         ...
         executeNurse: "",
          rules: {
          patientId: [
            { required: true, message: '请输入选择患者', trigger: 'change' },
          ],
          executeNurse: [
            { required: true, message: '请输入选择护士', trigger: 'change' }
          ],
          ifKnowInform: [
            { required: true, message: '是否已告知', trigger: 'change' }
          ],
            patientResponse: [
   ...
   created() {
      ...
      // this.form.patientId = this.patientList[0].patientId;
   ...
   getNursePatients() {
         ...
         if (data1.patientId) {
            ...
            this.form.patientId = data1.patientId
         } else {
            this.patientId = null;
         }
         this.getList();
      ...
      //    this.page.pageSize = 10
      //    if (val) {
      //       this.searchForm.executeNurse = val
      //    }else{
      //       this.searchForm.executeNurse = null
      //    }
      //    this.getList()
      // },
    //关闭表单验证
   ...
   cellStyle({ row, column, rowIndex, columnIndex }) {
      ...
      Submitconstraint(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
              //根据ID 找患者姓名
         for (var i in this.patientList) {
            if (this.patientList[i].patientId == this.form.patientId) {
               this.form.patientName = this.patientList[i].name;
            }
         }
      ...
      constraintDocument() {
         ...
         if (this.form.patientId.length > 0) {
